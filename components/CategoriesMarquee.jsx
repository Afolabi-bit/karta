      ...
      <div className="flex min-w-[200%] animate-[marqueeScroll_10s_linear_infinite] sm:animate-[marqueeScroll_40s_linear_infinite] group-hover:[animation-play-state:paused] gap-4">
        {[...categories, ...categories, ...categories, ...categories].map(
          (company, index) => (
            <button
              key={index}
              className="px-5 py-2.5 bg-slate-100/90 rounded-full text-slate-700 font-medium text-xs sm:text-sm hover:bg-[#002642] hover:text-white active:scale-95 transition-all duration-300 shadow-xs border border-slate-200/60"
            >
              {company}
            </button>
          ),
