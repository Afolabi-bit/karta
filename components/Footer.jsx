...
const Footer = () => {
  ...
  return (
    <footer className="mx-6 bg-white border-t border-slate-200 mt-20">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row items-start justify-between gap-10 py-12 border-b border-slate-200 text-slate-600">
          <div>
            ...
            <p className="max-w-102.5 mt-5 text-sm leading-relaxed text-slate-600">
              Welcome to Karta, your ultimate destination for the latest and
            ...
            <div className="flex items-center gap-3 mt-6">
              {socialIcons.map((item, i) => (
                <Link
                  href={item.link}
                  key={i}
                  className="flex items-center justify-center w-10 h-10 bg-slate-100 hover:bg-[#002642] hover:text-white hover:scale-105 transition-all rounded-full shadow-xs"
                >
                  <item.icon />
                </Link>
              ))}
          ...
          <div className="flex flex-wrap justify-between w-full md:w-[48%] gap-6 text-sm">
            {linkSections.map((section, index) => (
              <div key={index}>
                <h3 className="font-bold text-[#002642] md:mb-5 mb-3 tracking-wider text-xs uppercase">
                  {section.title}
                </h3>
        ...
        <p className="py-6 text-xs text-center md:text-left text-slate-500 font-medium">
          Copyright 2026 ©{" "}
          <span className="font-semibold text-[#002642]">Karta</span>. All
          Rights Reserved.
        </p>
      </div>
    </footer>
